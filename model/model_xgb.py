...
def model_tuner(args):
    ...
    xgb = XGBRegressor(learning_rate=para1, n_estimators=para2)
    #################################################
...
def model_evaluation(data, turn, train_idxes, test_idxes):
    ...
    ################# 要改模型改这里 #################
    xgb = XGBRegressor(learning_rate=best_lr, n_estimators=best_est)
    xgb.fit(X_train, Y_train)

    res = xgb.predict(X_test)
    fea_importance = xgb.feature_importances_
    #################################################
