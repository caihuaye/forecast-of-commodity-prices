...
def model_tuner(args):
    ...
    ################# 要改模型改这里 #################
    para1 = args['neighbour']
    knn = KNeighborsRegressor(n_neighbors=para1)
    #################################################
...
def model_evaluation(data, turn, train_idxes, test_idxes):
    ...
    ################# 要改模型改这里 #################
    space = {
        'neighbour': hp.choice('neighbour', [2, 3, 4, 5, 6, 7, 8, 9, 10])
    }
    #################################################
    ...
    ################# 要改模型改这里 #################
    para_neighbour = [2, 3, 4, 5, 6, 7, 8, 9, 10]

    best_neighbour = para_neighbour[best['neighbour']]
    best_para = [best_neighbour]
    #################################################

    # test & evaluate the model
    ################# 要改模型改这里 #################
    knn = KNeighborsRegressor(n_neighbors=best_neighbour)
    ...
    return best_para, res, Y_test, mse_value, R_value
...
if __name__ == "__main__":
    ...
    for iter in range(iter_time):
        ...
        for i in range(kfold_num):
            ...
            best_para, res_score, target_score, mse_val, corr_value = model_evaluation(
